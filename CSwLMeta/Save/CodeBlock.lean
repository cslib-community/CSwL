-- Adapted from /Users/ar/r/sf-in-lean/SFLMeta/Save/CodeBlock.lean
...
def diagramWithAlt : DirectiveExpanderOf Unit
  | (), contents => do
    ...
    ``(Verso.Doc.Block.other CSwLMeta.Block.diagramWithAlt #[$blocks,*])
...
chapter source's header preamble in `emitSavedImpl` (which also bundles
non-chapter prerequisites), so nothing is emitted here. -/
...
def importBlock : CodeBlockExpanderOf Unit
  | (), str => do
    ...
    ``(Verso.Doc.Block.other (CSwLMeta.Block.importBlock $(quote src))
