...
/-- A production whose alternatives are all bare single tokens with no gloss (e.g.
`column ::= A | B | ... | J`) reads better as one row than as ten. Not in the
upstream `sf-in-lean` version — SF's productions rarely have more than 2–3 such
alternatives, but `CSwFP`'s Sea Battle grammar (`column`, `row`) does. -/
def BnfProduction.isCompact (p : BnfProduction) : Bool :=
  p.alts.size > 1 && p.alts.all fun alt => alt.tokens.size == 1 && alt.note.isNone
...
/-- A term-level BNF literal: elaborates to a `CSwLMeta.BNF` value. -/
...
def tokToTerm (stx : TSyntax `bnfTok) : MacroM (TSyntax `term) :=
  ...
  | `(bnfTok| $i:ident) =>
    ...
    if name.startsWith "_" then
      ...
      `(CSwLMeta.BnfToken.meta $s)
    else
      let s := Syntax.mkStrLit name
      `(CSwLMeta.BnfToken.nonterm $s)
  | `(bnfTok| $s:str) =>
    `(CSwLMeta.BnfToken.lit $s)
...
def altToTerm (stx : TSyntax `bnfAlt) : MacroM (TSyntax `term) := do
  ...
  let noteTerm ← match note with
    | some s => `(some $s)
    | none   => `(none)
  `(({ tokens := #[$tokTerms,*], note := $noteTerm } : CSwLMeta.BnfAlt))
...
glosses; it is present but empty for a grammar that has none. A `BnfProduction.isCompact`
production collapses to a single row, its alternatives joined inline by `|`. -/
def toHtmlImpl (b : BNF) : Html :=
  let rows : Array Html := b.productions.flatMap fun p =>
    if p.isCompact then
      let altCell : Html :=
        Html.seq <| p.alts.foldl (init := #[]) fun acc alt =>
          if acc.isEmpty then #[altToHtml alt]
          else acc.push {{ <span class="bnf-sep-inline">{{" | "}}</span> }} |>.push (altToHtml alt)
      #[{{ <tr>
             <td class="bnf-lhs"><span class="bnf-nt">{{p.lhs}}</span></td>
             <td class="bnf-sep">{{"::="}}</td>
             <td class="bnf-alt">{{altCell}}</td>
             <td class="bnf-note">{{" "}}</td>
           </tr> }}]
    else
...
column is aligned across the whole grammar, not per production. A
`BnfProduction.isCompact` production collapses to a single line, its
alternatives joined by `|`. -/
def toTextImpl (b : BNF) : String :=
  let rows : Array (String × Option String) := b.productions.flatMap fun p =>
    if p.isCompact then
      #[(p.lhs ++ " ::= " ++ String.intercalate " | " (p.alts.toList.map altToText), none)]
    else
      p.alts.mapIdx fun i alt =>
...
carries the alternatives' glosses, and stays empty for a grammar that has none. A
`BnfProduction.isCompact` production collapses to a single row, its alternatives
joined inline by `$\mid$`. -/
def toTeXImpl (b : BNF) : Verso.Output.TeX :=
  let rows : Array Verso.Output.TeX := b.productions.flatMap fun p =>
    if p.isCompact then
      let altCell : Verso.Output.TeX :=
        .seq <| p.alts.foldl (init := #[]) fun acc alt =>
          if acc.isEmpty then #[altToTeX alt]
          else acc.push (.raw " $\\mid$ ") |>.push (altToTeX alt)
      #[.seq #[.raw s!"\\mathit\{{p.lhs}}", .raw " & ", .raw "::=", .raw " & ", altCell,
               .raw " & ", .empty, .raw " \\\\\n"]]
    else
      p.alts.mapIdx fun i alt =>
...
/* Separator between inline alternatives of a compact production (`column ::=
   A | B | ... | J` on one row) — not part of any token, so it stays unstyled
   apart from a little breathing room. */
table.bnf .bnf-sep-inline { padding: 0 0.3em; opacity: 0.65; }
...
def bnf : CodeBlockExpanderOf Unit
  | (), str => do
    ...
    ``(Verso.Doc.Block.other
        (CSwLMeta.Block.bnf $(quote json) $(quote src))
