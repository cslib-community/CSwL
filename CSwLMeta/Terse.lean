-- Adapted from sf-in-lean/SFLMeta/Terse.lean (namespace SFLMeta -> CSwLMeta).
import VersoManual
...
`CSwLMeta.Save.Extract` omits the heading comment from the generated terse
...
def terse : DirectiveExpanderOf Unit
  | (), contents => do
    ...
    ``(Verso.Doc.Block.other CSwLMeta.Block.terse #[$blocks,*])

@[directive]
def full : DirectiveExpanderOf Unit
  | (), contents => do
    let blocks ← contents.mapM elabBlock
    ``(Verso.Doc.Block.other CSwLMeta.Block.full #[$blocks,*])

@[directive]
def suppressPreviousHeaderWhenTerse : DirectiveExpanderOf Unit
  | (), _ =>
    ``(Verso.Doc.Block.other CSwLMeta.Block.suppressPreviousHeaderWhenTerse #[])
