...
Os exercícios deste capítulo vêm com *testes*, e os testes são escritos na
mesma linguagem. Um `example` enuncia uma afirmação sem lhe dar nome; o que
vem depois do `:=` é a justificativa. A tática `rfl` fecha uma igualdade
quando os dois lados *calculam* o mesmo valor — é o que confere se a
definição pedida faz o que se pediu. Um `theorem` é um `example` com nome,
e o nome serve para que o enunciado possa ser citado depois.
...
sozinha, é o assunto de {ref "Proof"}[Prova em Lean].
